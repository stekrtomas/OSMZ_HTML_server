...
public  class CameraServer {
    ...
    private  Semaphore safeShootSemaphore;
    ...
    public CameraServer(Camera camera) throws InterruptedException, IOException {
        this.camera = camera;
        this.sockets = new ArrayList<>();
        surfaceTexture = new SurfaceTexture(0);
        this.safeShootSemaphore = new Semaphore(1);
        picturePreviewCallback = (PreviewCallback) (new PreviewCallback() {
            public final void onPreviewFrame(final byte[] data, Camera cammera) {
                Thread th = new Thread(() -> {
                    if (CameraServer.this.safeShootSemaphore.tryAcquire()) {
                        Parameters parameters = CameraServer.this.camera.getParameters();
                        ...
                        try {
                            CameraServer.this.sendPhoto(out.toByteArray());
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                        CameraServer.this.safeShootSemaphore.release();
                    }
                });
                th.start();
            }
